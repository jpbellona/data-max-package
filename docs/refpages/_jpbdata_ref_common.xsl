...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:variable name="doctype" select="'refpage'"/>
	...
	<xsl:include href='../_jpbdata_platform.xsl'/>
	
  <xsl:variable name="jargon_enable">
    <xsl:choose>
      <xsl:when test="$compliant=1">
        <xsl:value-of select="document('../_jpbdata_common.xml')/root/setting[@name='jargon_enable']/@value"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:value-of select="document('../../_jpbdata_common.xml')/root/setting[@name='jargon_enable']/@value"/>
      </xsl:otherwise>
  ...
  <xsl:variable name="jargon_mouseover">
    <xsl:choose>
      <xsl:when test="$compliant=1">
        <xsl:value-of select="document('../_jpbdata_common.xml')/root/setting[@name='jargon_mouseover']/@value"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:value-of select="document('../../_jpbdata_common.xml')/root/setting[@name='jargon_mouseover']/@value"/>
      </xsl:otherwise>
  ...
  <xsl:template match="/">
    <html>
      <head>
        <link rel="stylesheet" type="text/css" href="../../_jpbdata_common.css"/>
  ...
  <xsl:template match="c74object">
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <!-- box attr link -->
        <xsl:if test="/c74object/@name != 'data' and not(/c74object/@box=0)">
          <h4>
            <a>
              <xsl:attribute name="href">
                <xsl:choose>
                  <xsl:when test="document(concat($refs_root_loc_local,'/max-ref/_jpbdata_contents.xml'))/root/refpage[@name='jbox.maxref.xml']">
                    <xsl:value-of select="concat($refs_root_link,'/max-ref/jbox.maxref.xml')"/>
                  </xsl:when>
                  <xsl:when test="document(concat($refs_root_loc_local,'/kernel-ref/_jpbdata_contents.xml'))/root/refpage[@name='jbox.maxref.xml']">
...
<!-- 
  ...
  <xsl:template match="mop">
    ...
    <!-- <xsl:value-of select="concat($refs_root_loc_local,'/jit-ref/_jpbdata_contents.xml')" /> -->
    <a>
      <xsl:attribute name="href">
        <xsl:choose>
          <xsl:when test="document(concat($refs_root_loc_local,'/jit-ref/_jpbdata_contents.xml'))/root/refpage[@name='jit.group-mop.maxref.xml']">
  ...
  <xsl:template match="OB3D">
    <div id="ob3d_section">
      <a>
        <xsl:attribute name="href">
          <xsl:choose>
            <xsl:when test="document(concat($refs_root_loc_local,'/jit-ref/_jpbdata_contents.xml'))/root/refpage[@name='jit.group-gl.maxref.xml']">
    ...
    <xsl:template match="parameter">
      <div id="parameter_section">
        <a>
          <xsl:attribute name="href">
            <xsl:choose>
              <xsl:when test="document(concat($refs_root_loc_local,'/m4l-ref/_jpbdata_contents.xml'))/root/refpage[@name='parameters.maxref.xml']">
